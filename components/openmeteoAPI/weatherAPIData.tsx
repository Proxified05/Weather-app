import { getGeocodingAPI } from "../geocodingAPI/geocodingAPIData";
export const getWeatherAPI = async (city: string) => {
    const { latitude, longitude } = await getGeocodingAPI(city);
    const weatherResponse = await fetch(
        `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&current=temperature_2m,weather_code,relative_humidity_2m,wind_speed_10m,visibility,uv_index&temperature_unit=celsius&daily=weather_code,temperature_2m_max,temperature_2m_min&timezone=auto&hourly=weather_code,temperature_2m`,
    );
    ...
    const dailyWeather = data.daily.time.slice(0, 7).map((time: number, index: number) => ({
        date: numToDay(new Date(time).getDay()),
        minTemp: data.daily.temperature_2m_min[index],
        maxTemp: data.daily.temperature_2m_max[index],
        weatherCode: data.daily.weather_code[index],
    }))
    return { data, currentWeather, dailyWeather, hourlyWeather };
}